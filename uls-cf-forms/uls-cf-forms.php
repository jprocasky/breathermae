...
/**
 * Resolve the EFFECTIVE user ID for CF operations.
 *
 * Priority:
 * 1) Submitted form field "user_id" (impersonated member)
 * 2) Logged-in user (provider or normal member)
 */
function ulscf_get_effective_user_id( $record = null ) {
    ...
    if (
        ...
        if (
            is_array( $fields_raw ) &&
            isset( $fields_raw['user_id']['value'] ) &&
            ctype_digit( (string) $fields_raw['user_id']['value'] )
        ) {
            $candidate = (int) $fields_raw['user_id']['value'];

            if ( get_user_by( 'id', $candidate ) ) {
    ...
    // Fallback: real logged‑in user
    return get_current_user_id();
}

function ulscf_get_effective_email( $record = null ) {
    global $email_address;
    // 1️⃣ Preferred source: Elementor submission payload
    if (
        $record &&
        method_exists( $record, 'get' )
    ) {
        ...
        if (
            is_array( $fields_raw ) &&
            isset( $fields_raw['email']['value'] ) &&
            is_email( $fields_raw['email']['value'] )
        ) {
            // Optional hardening: ensure it belongs to a real user
            $candidate = sanitize_email( $fields_raw['email']['value'] );
            if ( get_user_by( 'email', $candidate ) ) {
    ...
    // 2️⃣ Fallback: logged-in user email
    $current_user = wp_get_current_user();
    if ( $current_user && ! empty( $current_user->user_email ) ) {
          $email_address = sanitize_email( $current_user->user_email );
...
add_action(
  ...
  function( $record, $handler = null, $ajax_handler = null ) {
    ...
    try {
      ...
      $effective_user_id = ulscf_get_effective_user_id( $record );
      $email             = ulscf_get_effective_email( $record );

      if ( ! $effective_user_id || empty( $email ) ) {
