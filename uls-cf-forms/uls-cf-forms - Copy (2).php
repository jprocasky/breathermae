...
function ulscf_ensure_table( $form_name, $fields_raw ) {
  ...
  foreach ( $fields_raw as $id => $field ) {
    ...
    // ✅ Skip internal / control-only fields
    if ( $id === 'uls_scoped_member_id' ) {
        continue;
    }
...
function ulscf_get_effective_user_id() {

    $uid = get_current_user_id();

    if (
        isset( $_GET['uls_view_as'] ) &&
        ctype_digit( (string) $_GET['uls_view_as'] )
    ) {
        $member_id = (int) $_GET['uls_view_as'];
        if ( get_user_by( 'id', $member_id ) ) {
            return $member_id;
        }
    }
...
add_action(
  ...
  function( $record, $handler = null, $ajax_handler = null ) {
    ...
    try {
      ...
      if ( 0 !== strpos( $form_name, ULS_CF_PREFIX ) ) {
        return;
      }
      ...
      if ( empty( $fields_raw ) || ! is_array( $fields_raw ) ) {
        return;
          ...
          /*
          * ---------------------------------------------------------
          * Resolve EFFECTIVE user (provider or scoped member)
          * ---------------------------------------------------------
          */

          $effective_user_id = ulscf_get_effective_user_id();
          $effective_user    = get_user_by( 'id', $effective_user_id );

          if ( ! $effective_user || empty( $effective_user->user_email ) ) {
              return;
          }

          $email = sanitize_email( $effective_user->user_email );
      ...
      $data = array(
        'email'   => $email,
        'user_id' => intval( $effective_user_id ),
      );
      ...
      if ( ulscf_should_insert_new( $fields_raw, $latest_any ) || ! $latest_unlocked ) {
        ...
        $wpdb->update(
          $table_name,
          $data,
          array( 'id' => intval( $latest_unlocked->id ) )
        );
      }
...
add_shortcode( 'uls_cf_value', function( $atts ) {
  ...
  if ( empty( $atts['form'] ) || empty( $atts['field'] ) ) return $atts['default'];
    ...
    // Scoped impersonation (read-as member)
    if ( empty( $email ) && isset( $_GET['uls_view_as'] ) && ctype_digit( (string) $_GET['uls_view_as'] ) ) {
        $member = get_user_by( 'id', (int) $_GET['uls_view_as'] );
        if ( $member && ! empty( $member->user_email ) ) {
            $email = sanitize_email( $member->user_email );
        }
    }
...
add_filter(
    ...
    function ( $item, $index, $form ) {
        ...
        if ( strpos( $form_name, ULS_CF_PREFIX ) !== 0 ) {
            return $item;
        }
        ...
        // Respect editor-provided defaults
        if ( ulscf_has_editor_default( $item ) ) {
            return $item;
        }

        /*
         * ---------------------------------------------------------
         * Resolve EFFECTIVE email (scoped impersonation aware)
         * ---------------------------------------------------------
         */
        ...
        if ( ! in_array( $type, $single_types, true ) ) {
            return $item;
        }
...
add_filter(
    ...
    function ( $item, $index, $form ) {
        ...
        $is_multiple = ulscf_is_item_multiple( $item )
            || ( is_array( $val ) && count( (array) $val ) > 1 );
