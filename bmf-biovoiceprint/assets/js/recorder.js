...
(function () {
  ...
  function initRecorder(root) {
    ...
    function resetPreview() {
      blob = null; chunks = []; recordedMime = ''; peakRms = 0;
      if (playerEl) { playerEl.removeAttribute('src'); playerEl.load(); }
    ...
    function startMeter(mediaStream) {
      ...
      try {
        ...
        const tick = function () {
          ...
          if (meterBar) { meterBar.style.width = pct + '%'; meterBar.classList.toggle('is-hot', pct > 85); }
    ...
    function getSavedDeviceId() { try { return localStorage.getItem(STORAGE_KEY) || ''; } catch (e) { return ''; } }
    function saveDeviceId(id) { try { if (id) localStorage.setItem(STORAGE_KEY, id); else localStorage.removeItem(STORAGE_KEY); } catch (e) {} }
    ...
    function micLabel() {
      if (deviceSel && deviceSel.selectedOptions && deviceSel.selectedOptions[0]) return deviceSel.selectedOptions[0].textContent || '';
      return '';
    }

    function buildDevicePayload() {
      return { device_class: detectDeviceClass(), os: detectOS(), browser: navigator.userAgent || '', mic_label: micLabel(), device_id: selectedDeviceId || '' };
    }
    ...
    async function refreshDevices() {
      ...
      if (labeled.length < 2) { deviceWrap.hidden = true; return; }
      ...
      if (saved && labeled.some(function (d) { return d.deviceId === saved; })) {
        deviceSel.value = saved; selectedDeviceId = saved;
      } else {
    ...
    async function start() {
      hideMessage(); resetPreview(); root.classList.remove('is-recording');
      ...
      if (deviceSel && !deviceWrap.hidden && deviceSel.value) {
        selectedDeviceId = deviceSel.value; saveDeviceId(selectedDeviceId);
      }
      try {
      ...
      mediaRecorder.onstop = function () {
        ...
        if (isMicCheck) {
          ...
          if (deviceSel) deviceSel.disabled = false;
          blob = null;
          try { root.dispatchEvent(new CustomEvent('bmf-biovoice-mic-check-passed', { bubbles: true, detail: { task_code: 'mic_check', peak_rms: peakRms } })); } catch (e) {}
          return;
        }
        if (minSec > 0 && elapsed + 0.05 < minSec) {
          ...
          if (previewEl) { const url = URL.createObjectURL(blob); if (playerEl) playerEl.src = url; previewEl.hidden = false; }
      ...
      timerId = setInterval(function () {
        ...
        if (maxSec > 0 && elapsed >= maxSec) stop();
      }, 200);
    ...
    async function save() {
      if (isMicCheck) { showMessage('Mic check does not need to be saved.', 'info'); return; }
      if (!blob) { showMessage('Nothing to save.', 'error'); return; }
      ...
      btnSave.disabled = true; setStatus('Uploading…'); hideMessage();
      ...
      try {
        ...
        if (!res.ok) {
          ...
          setStatus('Upload failed'); btnSave.disabled = false; return;
        }
        showMessage('Recording saved (session #' + (data.session_id || '?') + ').', 'success');
        setStatus('Saved'); resetPreview(); btnSave.disabled = false;
        try { root.dispatchEvent(new CustomEvent('bmf-biovoice-saved', { bubbles: true, detail: data })); } catch (e) {}
      } catch (err) {
        showMessage('Network error while saving. Please try again.', 'error');
        setStatus('Upload failed'); btnSave.disabled = false;
      }
    }

    function discard() { resetPreview(); setStatus('Ready to record'); hideMessage(); }

    if (deviceSel) {
      deviceSel.addEventListener('change', function () {
        selectedDeviceId = deviceSel.value || null; saveDeviceId(selectedDeviceId);
      });
    }
