...
class BMF_BioVoice_Shortcodes_Report {
	...
	public static function shortcode_report( $atts ) {
		...
		$meta   = $resolved['meta'];
		$overall = isset( $report['overall_result'] ) && is_array( $report['overall_result'] )
			? $report['overall_result']
			: [];
		$findings = isset( $report['key_findings'] ) && is_array( $report['key_findings'] )
			? $report['key_findings']
			: [];
		$tips = isset( $report['before_you_speak_or_present'] ) && is_array( $report['before_you_speak_or_present'] )
			? $report['before_you_speak_or_present']
			: [];
		$watch = isset( $report['what_to_watch_over_time'] ) && is_array( $report['what_to_watch_over_time'] )
			? $report['what_to_watch_over_time']
			: [];
		$preventive = isset( $report['preventive_wellness_insight'] ) && is_array( $report['preventive_wellness_insight'] )
			? $report['preventive_wellness_insight']
			: [];
		$scores = isset( $report['customer_facing_scores'] ) && is_array( $report['customer_facing_scores'] )
			? $report['customer_facing_scores']
			: [];
		...
		<article class="<?php echo esc_attr( $class ); ?>" data-bmf-biovoice-report data-source="<?php echo esc_attr( $meta['source'] ); ?>">
				...
				<section class="bmf-bv-report-section">
					...
					<ul class="bmf-bv-report-score-list">
						...
						foreach ( $score_keys as $sk => $slabel ) :
							if ( empty( $scores[ $sk ] ) || ! is_array( $scores[ $sk ] ) ) {
								continue;
							}
							$s        = $scores[ $sk ];
							...
							<li>
								...
								<span class="bmf-bv-report-score-val">
									<?php if ( isset( $s['score'] ) ) : ?>
										<strong><?php echo esc_html( number_format_i18n( (float) $s['score'], 1 ) ); ?></strong>
									<?php endif; ?>
									<?php if ( $tag_label ) : ?>
										<span class="bmf-bv-report-score-tag <?php echo esc_attr( $tag_mod ); ?>"><?php echo esc_html( $tag_label ); ?></span>
									<?php endif; ?>
								</span>
							</li>
						<?php endforeach; ?>
						...
						<?php foreach ( $tips as $tip ) :
							...
							<li>
								...
								<?php if ( ! empty( $tip['explanation'] ) ) : ?>
									<span><?php echo esc_html( $tip['explanation'] ); ?></span>
								<?php endif; ?>
							</li>
						<?php endforeach; ?>
					</ul>
			...
			<?php if ( $preventive || $watch ) : ?>
				<section class="bmf-bv-report-section">
					...
					<?php if ( ! empty( $preventive['summary'] ) ) : ?>
						<p><?php echo esc_html( $preventive['summary'] ); ?></p>
					<?php endif; ?>
					<?php if ( ! empty( $preventive['context'] ) ) : ?>
						<p class="bmf-bv-report-muted"><?php echo esc_html( $preventive['context'] ); ?></p>
					<?php endif; ?>
					<?php if ( ! empty( $watch['summary'] ) ) : ?>
						<p><?php echo esc_html( $watch['summary'] ); ?></p>
					<?php endif; ?>
			...
			<?php if ( ! empty( $report['safe_disclaimer'] ) ) : ?>
				<footer class="bmf-bv-report-disclaimer">
					<?php echo esc_html( $report['safe_disclaimer'] ); ?>
				</footer>
			<?php endif; ?>
		</article>
		<?php
