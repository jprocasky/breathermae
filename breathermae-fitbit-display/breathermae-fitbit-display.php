...
class BM_Fitbit_Display {
    ...
    public function bootstrap(){
      ...
      add_shortcode('bm_fitbit_sleep_avg_asleep', function ($atts) {
          ...
          $user_id = $a['user_id'] !== '' ? intval($a['user_id']) : get_current_user_id();
          if (!$user_id) {
              return '';
          }
          ...
          // Use only completed days (exclude today)
          $end_date   = date('Y-m-d', strtotime('-1 day', current_time('timestamp')));
          $start_date = date(
              'Y-m-d',
              strtotime('-' . ($days - 1) . ' days', strtotime($end_date))
          );

          $avg_minutes = $wpdb->get_var(
              $wpdb->prepare(
                  "SELECT AVG(minutes_asleep)
                  FROM {$table}
                  WHERE wp_user_id = %d
                    AND date_of_sleep BETWEEN %s AND %s",
                  $user_id,
                  $start_date,
                  $end_date
              )
          ...
          $avg_minutes = (int) round($avg_minutes);
          $hours   = intdiv($avg_minutes, 60);
          $minutes = $avg_minutes % 60;

          return sprintf('%d hr %d min', $hours, $minutes);
      });

      add_shortcode('bm_fitbit_sleep_avg_efficiency', function ($atts) {
          ...
          if ($avg_efficiency === null) {
              return '—';
          }

          return round($avg_efficiency) . '%';
      });
    
      add_shortcode('bm_fitbit_rhr_avg', function ($atts) {
          ...
          $avg_rhr = $wpdb->get_var(
              $wpdb->prepare(
                  "SELECT AVG(resting_bpm)
                  FROM {$table}
                  WHERE wp_user_id = %d
                    AND rhr_date BETWEEN %s AND %s",
                  $user_id,
                  $start_date,
                  $end_date
              )
          ...
          return round($avg_rhr) . ' bpm';
      });    
      
      add_shortcode('bm_fitbit_rhr_low', function ($atts) {
          ...
          $min_rhr = $wpdb->get_var(
              $wpdb->prepare(
                  "SELECT MIN(resting_bpm)
                  FROM {$table}
                  WHERE wp_user_id = %d
                    AND rhr_date BETWEEN %s AND %s",
                  $user_id,
          ...
          return $min_rhr !== null ? intval($min_rhr) . ' bpm' : '—';
      });    
      add_shortcode('bm_fitbit_rhr_high', function ($atts) {
          ...
          $max_rhr = $wpdb->get_var(
              $wpdb->prepare(
                  "SELECT MAX(resting_bpm)
                  FROM {$table}
                  WHERE wp_user_id = %d
                    AND rhr_date BETWEEN %s AND %s",
                  $user_id,
          ...
          return $max_rhr !== null ? intval($max_rhr) . ' bpm' : '—';
      });    

add_shortcode('bm_fitbit_steps_total', function ($atts) {
    ...
    // Completed days only (exclude today)
    $end_date = date('Y-m-d', strtotime('-1 day', current_time('timestamp')));
    $start_date = date(
        'Y-m-d',
        strtotime('-' . ($days - 1) . ' days', strtotime($end_date))
    ...
    return number_format_i18n((int)$total_steps) . ' steps';
});

add_shortcode('bm_fitbit_steps_avg', function ($atts) {
    ...
    return number_format_i18n(round($avg_steps)) . ' steps/day';
});

add_shortcode('bm_fitbit_distance_total', function ($atts) {
          ...
          <div class="bmfd-table-wrap">
            <table class="bmfd-table" role="table" aria-label="Activity detail">
              ...
              <tbody>
                <?php foreach($rows as $r): ?>
                  <tr>
                    <td style="white-space: nowrap;"><?php echo esc_html($r['activity_date']); ?></td>
