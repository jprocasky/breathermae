...
/*         // ----------------------------
        ...
        $data['formula_meta'] = $formula_meta; */

/*         // ----------------------------
        // ✅ NEW: Redirect Mapping (meta_json)
        // ----------------------------
        $meta = [];

        // Load existing meta_json if it exists
        if ( ! empty( $_POST['meta_json'] ) ) {
            $meta = json_decode( wp_unslash( $_POST['meta_json'] ), true );
            if ( ! is_array( $meta ) ) {
                $meta = [];
            }
        }
        ...
        // Only set if user entered something
        if ( ! empty( $redirects ) ) {
        ...
        // ✅ Persist meta_json
        if ( ! empty( $meta ) ) {
            $data['meta_json'] = wp_json_encode( $meta );
        } */

                // ----------------------------
                // ✅ NEW: JSON Merge System
                // ----------------------------

                // ✅ Load raw JSON (from textarea inputs)
                ...
                // ----------------------------
                // ✅ Redirect Mapping (UI overrides JSON)
                // ----------------------------
                $redirects = [];
                ...
                // ✅ Save merged JSON
                // ----------------------------
                $data['formula_meta'] = !empty($formula_meta)
                    ? wp_json_encode($formula_meta)
                    : null;

                $data['meta_json'] = !empty($meta)
                    ? wp_json_encode($meta)
                    : null;
