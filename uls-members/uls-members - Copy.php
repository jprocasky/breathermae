...
class ULS_Members_Plugin {
    ...
    // Attributes: keep defaults so existing pages render the same
        $atts = shortcode_atts(
            [
                'per_page' => 10,
                'fields'   => '',
    ...
    // Allowed fields + legacy defaults (current behavior)
    $allowed_fields  = [ 'email', 'display_name', 'first_visit', 'last_visit', 'matched_tags' ];
    $default_fields  = [ 'email', 'display_name', 'first_visit', 'last_visit', 'matched_tags' ];

    // Parse requested fields (if any), sanitize, and intersect with allowed
    ...
    // If nothing valid made it through, fall back to defaults
    if ( empty( $fields ) ) {
        $fields = $default_fields;
    }

    // Build header labels (either auto from map or custom via headers=)
    $labels_map = [
        'email'        => 'Email',
        'display_name' => 'Name',
    ...
    if ( (string) $atts['headers'] !== '' ) {
        ...
        foreach ( $fields as $f ) { $headers[] = $labels_map[ $f ]; }
    }
    ...
    if ( ! empty( $override_patterns ) ) {
        ...
        // ✅ SHORTCODE OVERRIDE PATH
        $child_patterns = $override_patterns;

    } else {

        // ✅ EXISTING TABLE-BASED PATH
    ...
    // 4) Enrich with first/last visits
    ...
    <div class="uls-members" data-per-page="<?php echo esc_attr( $per_page ); ?>">
        <div class="uls-members__search">
            <input
                type="text"
                class="uls-members__search-input"
                placeholder="Search members…"
                autocomplete="off"
            />
            <button
                type="button"
                class="uls-members__search-clear"
        ...
        <?php foreach ( $rows as $r ): ?>
            ...
            <?php foreach ( $fields as $f ): ?>
                <?php
                // Map friendly field names to actual array keys when needed
                $key = $f;
                if ( $f === 'email' ) {
                    $key = 'user_email';
                }
                ...
                if ( $f === 'matched_tags' ) {
                    ...
                    $cell = isset( $r[ $key ] ) ? (string) $r[ $key ] : '';
                }

                // Add data-col="email" only for the email column (so JS can find it)
