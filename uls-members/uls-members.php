...
class ULS_Members_Plugin {
    ...
    public function enqueue_assets() {
        ...
        wp_register_style( 'uls-members-css', plugins_url( 'uls-members.css', __FILE__ ), [], '1.8.2' );
        wp_enqueue_style( 'uls-members-css' );

        // JS for row selection + AJAX + pagination + tag admin + claim
        wp_register_script( 'uls-members-js', plugins_url( 'uls-members.js', __FILE__ ), [ 'jquery' ], '1.8.2', true );
    ...
    /**
     * Default toggle list when [uls_member_tag_admin] has no tags="" attr.
     * Keys are WP Fusion tag labels; values are checkbox captions.
     */
    private function get_default_simple_tag_map() {
        return [
    ...
    private function get_simple_tag_map() {
        return $this->get_default_simple_tag_map();
    }

    /**
     * True if a toggle slug is safe to apply/remove via tag-admin.
     * Parent sales codes (SA###) stay on the Sales Person controls.
     */
    private function is_allowed_simple_tag_slug( $tag ) {
        $tag = trim( (string) $tag );
    ...
    /**
     * Parse shortcode / AJAX tags list.
     * Format: "Label|TAG, Label|TAG" (comma-separated).
     * If a pair has no pipe, the token is used as both label and tag.
     *
    ...
    private function parse_simple_tag_map( $raw ) {
        ...
        foreach ( explode( ',', $raw ) as $pair ) {
            ...
            if ( ! $this->is_allowed_simple_tag_slug( $tag ) ) {
                continue;
            }
            if ( $label === '' ) {
                $label = $tag;
        ...
        return ! empty( $map ) ? $map : $this->get_default_simple_tag_map();
    }

    /** Map sent by the tag-admin panel (data-tags), else defaults. */
    private function resolve_simple_tag_map_from_request() {
        $raw = '';
        if ( isset( $_POST['tags'] ) ) {
            $raw = sanitize_text_field( wp_unslash( $_POST['tags'] ) );
        }
        return $this->parse_simple_tag_map( $raw );
    }
     ...
     *
     * @param int        $user_id
     * @param array|null $tag_map tag => label; null = defaults
     */
    private function build_tag_admin_status( $user_id, $tag_map = null ) {
        ...
        if ( ! is_array( $tag_map ) || empty( $tag_map ) ) {
            $tag_map = $this->get_default_simple_tag_map();
        }
    ...
    public function ajax_get_tag_admin_status() {
        ...
        $status = $this->build_tag_admin_status( $user_id, $this->resolve_simple_tag_map_from_request() );
        if ( ! $status ) {
    ...
    public function ajax_toggle_simple_tag() {
        ...
        $tag_map = $this->resolve_simple_tag_map_from_request();
    ...
    public function ajax_make_sales_person() {
        ...
        wp_send_json_success( $this->build_tag_admin_status( $user_id, $this->resolve_simple_tag_map_from_request() ) );
    }
     ...
     *
     * Optional tags="" list controls the toggle checkboxes:
     *   [uls_member_tag_admin tags="Employee Access|INTERNAL, 8 Pillars|SURVEYS, Art of Wellness|ART, 360 Health|S360, Provider|DOCTOR"]
     *
     * Format: Label|TAG, comma-separated. Listens to uls:selected-member.
     */
    public function shortcode_member_tag_admin( $atts ) {
        ...
        $atts = shortcode_atts(
            ...
            $atts,
            'uls_member_tag_admin'
        );

        $tags_raw = trim( (string) $atts['tags'] );
        $tag_map  = $this->parse_simple_tag_map( $tags_raw );
        if ( $tags_raw === '' ) {
            $pairs = [];
            foreach ( $tag_map as $tag => $label ) {
                $pairs[] = $label . '|' . $tag;
            }
            $tags_raw = implode( ', ', $pairs );
        }
        ...
        <div class="uls-tag-admin" id="uls-tag-admin"
             data-user-id=""
             data-email=""
             data-tags="<?php echo esc_attr( $tags_raw ); ?>">
